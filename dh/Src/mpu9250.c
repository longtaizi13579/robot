...
/*
  错误处理，当I2C错误累计到一定次数，复位I2C，以防止I2C死锁
*/
inline void mpu_error_deal(MPU_Dev *dev){
		dev->i2c_error_count++;
...
void MPU9250_Init(MPU_Dev * dev){
	...
	dev->setting->accel_range_setting=RANGE2G;    //更改此处可更改加速度计量程
	dev->setting->accel_high_pass_filter_setting=_5HZ;
	dev->setting->gyro_range_setting=RANGE500;    //更改此处可更改陀螺仪量程
    ...
    if(dev->dev_ID==0x70||dev->dev_ID==0x68){
      //mpu6500
      dev->MAG_OK=0;
    }else{
      dev->MAG_OK=1;
    }
	if(dev->dev_ID!=0x71&&dev->dev_ID!=0x70&&dev->dev_ID!=0x68&&dev->dev_ID!=0x73){
      //如果既不是MPU6050 MPU6500 MPU9250
		dev->i2c_reset();
